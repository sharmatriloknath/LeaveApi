...
def login(request):
    ...
    print("the value inside the uri is", 'http://' + request.get_host() + '/LMS/TheSeekers/complete/auth0')
...
def profile(request):
    ...
    print("The value in the reqest.gethost", request.get_host(), userinfo)
    print("The role of the employee is:--------------------",userinfo['app_metadata']['role'])
...
def Home(request):
    ...
    print("The value in the reqest.gethost",request.get_host(), userinfo)
    employee_list = Employee.objects.get(Email_Address=userinfo['email'])
    print("the employee is", employee_list)
